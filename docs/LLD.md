...
**Layer 2 — Airflow DAGs (hours).** DAG 2 validates data quality and produces a training candidate; the Validation Pipeline validates model performance via the champion/challenger gate; DAG 3 validates production behavior via shadow deployment with a manual approval gate before canary.
...
The key difference from traditional software CI/CD: it asks "did the tests pass?" ML CI/CD asks "is this model actually better than what's in production?" — the champion/challenger comparison gate in the Validation Pipeline and the shadow-evaluation comparison in DAG 3 are the ML-specific steps with no equivalent in standard software deployment.
